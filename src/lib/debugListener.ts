...
function push(label: string, detail: string) {
  ...
  // Ship to server log file (fire-and-forget)
  fetch('/api/debug-log', {
    method: 'POST',
...
function describeChange(c: { key: string; from: unknown; to: unknown }): { label: string; detail: string } {
  const { key, from, to } = c;
  ...
  if (key === 'agents') {
    ...
    const changed = [...nextIds].filter(id => prevIds.has(id) && prevAgents[id] !== nextAgents[id]);

    const parts: string[] = [];
    if (added.length) parts.push(`+${added.length} added (${added.slice(0, 3).join(', ')}${added.length > 3 ? '...' : ''})`);
  ...
  try {
    return { label: key, detail: `${JSON.stringify(from).slice(0, 80)} → ${JSON.stringify(to).slice(0, 80)}` };
  } catch {
...
export function startDebugListener() {
  ...
  console.log('%c[DEBUG] Listener active — tracking all store mutations', 'color: #0f0; font-weight: bold');
}
