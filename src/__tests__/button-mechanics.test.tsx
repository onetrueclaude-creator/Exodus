...
describe('TabNavigation', () => {
  ...
  it('previously active tab loses accent-cyan class when another is selected', () => {
    ...
    expect(screen.getByText('Network').closest('button')!.className).not.toContain('text-accent-cyan');
  });
...
describe('ResourceBar', () => {
  ...
  it('renders agent username from store', () => {
    render(<ResourceBar />);
    expect(screen.getByText('TestAgent')).toBeDefined();
  });

  it('renders "Your Network" when no agent is selected', () => {
    useGameStore.getState().reset();
    render(<ResourceBar />);
    expect(screen.getByText('Your Network')).toBeDefined();
  });

  it('renders agent tier label', () => {
    render(<ResourceBar />);
    // makeAgent uses 'sonnet' tier
