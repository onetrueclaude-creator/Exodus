...
- 2026-03-28: Public API deployment at `api.zkagentic.ai` (Railway, pending deploy). New Supabase tables: `subgrid_allocations` (per-wallet cell counts), `resource_rewards` (per-wallet yields) — both with RLS + Realtime publication. Service_role key moved from hardcoded to env vars.
