...
class GrowthSimulator:
    ...
    def _compute_vesting_schedule(months: int) -> list[float]:
        ...
        # TODO(v2): redesign — v2 has no pre-minted vesting allocations.
        """
        from agentic.params import (
        ...
        schedules = [
            ...
            (DIST_COMMUNITY, 12, 48, 0),      # 25% — locked 1yr, 4yr vest
            (DIST_PROFESSIONAL, 6, 48, 0),     # 25%
            (DIST_FOUNDERS, 12, 48, 0),        # 25%
            (DIST_MACHINES, 0, 36, 0),         # 25%
        ]
        ...
        for month in range(months + 1):
            ...
            for share, cliff, vest, genesis in schedules:
                allocation = _LEGACY_TOTAL_SUPPLY * share
                if month < cliff:
                    total += genesis
                elif vest == 0:
                    total += allocation
    ...
    def run(self) -> list[MonthlySnapshot]:
        ...
        circulating = float(_LEGACY_INITIAL_CIRCULATING)
