...
from agentic.params import FEE_BURN_RATE

# Legacy constant — removed from params.py in v2 (organic growth model)
_INITIAL_CIRCULATING = 6_300_000
...
class TestGrowthSimulator:
    ...
    def test_initial_circulating(self):
        sim = GrowthSimulator(GrowthScenario.baseline())
        snapshots = sim.run()
        # Month 0 circulating starts at legacy INITIAL_CIRCULATING
        assert snapshots[0].circulating_supply == _INITIAL_CIRCULATING
