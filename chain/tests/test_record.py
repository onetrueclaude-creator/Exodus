...
class TestRecord:
    ...
    def test_nullifier_requires_nullifier_key(self):
        r = Record(
            owner=b"a" * 32, data=[500], nonce=b"n" * 32,
            tag=b"t" * 32, program_id=b"prog", birth_slot=0,
        )
        nf = r.nullifier(nullifier_key=b"sk" + b"\x00" * 30)
        assert len(nf) == 32
        nf2 = r.nullifier(nullifier_key=b"sk2" + b"\x00" * 29)
        assert nf != nf2
...
class TestRecordNullifierKey:
    def test_nullifier_with_nullifier_key(self):
        """Record.nullifier() should accept nullifier_key parameter."""
        record = Record(
            owner=b"\x01" * 32,
            data=[100],
            nonce=b"\x03" * 32,
        ...
        nk = b"\x10" * 32
        nf = record.nullifier(nk)
        assert isinstance(nf, bytes)
        assert len(nf) == 32

    def test_nullifier_deterministic(self):
        """Same nullifier_key produces same nullifier."""
        record = Record(
            owner=b"\x01" * 32,
            data=[100],
            nonce=b"\x03" * 32,
        ...
        nk = b"\x10" * 32
        nf1 = record.nullifier(nk)
        nf2 = record.nullifier(nk)
        assert nf1 == nf2

    def test_nullifier_key_positional(self):
        """nullifier_key can be passed as positional arg."""
        record = Record(
            owner=b"\x01" * 32,
            data=[100],
            nonce=b"\x03" * 32,
        ...
        nk = b"\x10" * 32
        nf_positional = record.nullifier(nk)
        nf_keyword = record.nullifier(nullifier_key=nk)
        assert nf_positional == nf_keyword
