...
def _fetch_drand() -> tuple[int, bytes] | None:
    ...
    try:
        # B310 justification: fixed https constant, never user input
        with urllib.request.urlopen(_DRAND_URL, timeout=BEACON_HTTP_TIMEOUT_S) as r:  # nosec B310
...
def _fetch_solana_blockhash() -> bytes | None:
    ...
    try:
        ...
        # B310 justification: fixed https constant, never user input
        with urllib.request.urlopen(req, timeout=BEACON_HTTP_TIMEOUT_S) as r:  # nosec B310
            data = json.loads(r.read())
