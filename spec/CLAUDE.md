...
## 2026-07-16 — Whitepaper v1.7 (Finality Single-Path Revision)

**Changed:** `whitepaper.md` from v1.6 to v1.7 (July 2026), alongside the PR #222 consensus change it documents. One live consensus-behaviour change: verifier-committee selection on the live coordinator path (`pipeline._select_verifiers`, wired via `/api/mine`) now uses the token-only `finality_weight` (W_fin) — the v1.5-specified finality firewall — replacing the CPU-weighted `effective_stake` it had continued to use (previously disclosed in §13.5 Honest status). `S_eff` is preserved unchanged as the **economic** weight (reward share / earnings). Leader selection clarified as never-live-weighted (the testnet coordinator is a single fixed proposer; weighted scheduling = a mainnet item). ~20 disclosure sites reconciled (Abstract/§1.3, §5.5, §8, §13.5, §23.3 + related committee/selection mentions); the verification module's proving-target docstring aligned to Groth16/PLONK per §6.4/§21.1 (the STARK reference removed — the whitepaper never specified STARK). **§22 unchanged** (selection-source change, not a parameter change) — the concordance suite stays green. Companion current-version citation updates in the same PR: root `CLAUDE.md`, `ROADMAP.md`, and the litepaper (v1.7 lockstep, content unchanged). Security basis: CPU / Proof-of-Vault-derived work is Sybil-weak until PoRep-hardened; the firewall excludes it from finality while keeping it on earnings. *(Source: PR #222 + the whitepaper's own v1.7 header paragraph.)*
