# AGNTC Whitepaper v1.7

> **ZK Agentic Chain: A Privacy-Preserving Blockchain with AI-Powered Verification**
>
> Version 1.7 (Finality Single-Path Revision) | July 2026
>
> v1.7 supersedes v1.6. It is a **consensus-honesty revision** with one live consensus-behaviour change: **verifier-committee selection on the live coordinator path now uses the token-only `finality_weight` (W_fin)** — the finality firewall v1.5 specified — replacing the CPU-weighted `effective_stake` that the live path had continued to use (which v1.6's §13.5 Honest status disclosed). `S_eff = α·token + β·cpu` is preserved unchanged as the **economic** weight (reward share / earnings proportionality) — the firewall separates *finality* from *earnings*. Leader selection is clarified, not changed: the testnet coordinator is a single fixed proposer — no weighted leader mechanism was ever live — and weighted leader scheduling remains a mainnet item (§13.5 Honest status). ~20 disclosure sites (Abstract/§1.3, §5.5, §8, §13.5, §23.3 and related committee/selection mentions) are reconciled to this single-path reality, and the verification module's proving-target reference is aligned to Groth16/PLONK per §6.4/§21.1. **§22 is unchanged** — a selection-source change, not a parameter change — so the concordance suite stays green. Security basis: CPU / Proof-of-Vault-derived work remains **Sybil-weak until PoRep-hardened**; excluding it from finality selection while keeping it on earnings is the firewall's purpose.
>
...
The protocol employs a dual-staking model that, for **economic** purposes, weights computational contribution (60%) over capital (40%), reducing plutocratic concentration in earnings inherent in pure proof-of-stake designs. Validators commit both AGNTC tokens and CPU compute resources; the effective stake `S_eff = α·token + β·cpu` is a weighted combination of both dimensions and governs **reward share / earnings proportionality** — CPU work earns proportionally more. **Consensus finality is firewalled from CPU.** Because CPU / Proof-of-Vault work is Sybil-weak until it is PoRep-hardened, the **finality weight** — committee (verifier) selection — is **AGNTC token-stake only** (the *finality firewall*, Section 13.5 Honest status), live in the coordinator's committee selection; leader selection has no live weighted mechanism yet (a single fixed proposer), pending mainnet leader-scheduling. Security is two-layered: a 13-agent PoAIV committee secures the *ledger*, while participants' committed CPU and disk secure the *state* — the collective knowledge vault — through Proof-of-Vault sampled storage proofs.
...
| W_fin(i) | Finality weight of validator i = T_i/T_total (**token stake only**; the finality firewall, §13.5 Honest status) — live for committee selection; leader selection pending |
...
**Layer 4 — Agent Layer.** AI verification agents are instantiated from Claude model variants organized into three tiers: Haiku (fast, low-cost inference for high-throughput verification), Sonnet (balanced reasoning for standard verification tasks), and Opus (deep reasoning for complex cross-ledger audits and anomaly detection). A committee of k=13 agents is selected per block via a verifiable random function weighted by **token stake** (the finality firewall, Section 13.5 — live: the coordinator-run testnet now selects committees on the token-only finality weight `W_fin`; CPU no longer weighs into committee selection). Each agent independently audits the proposed block and produces an attestation.
...
**Proportionality.** *Earnings* are proportional to demonstrated contribution, not just capital. The dual-staking model (Section 13) ensures that CPU compute — actual work performed — weighs more heavily than token holdings in determining **reward share**. Influence over committee-selection *finality* is, by contrast, proportional to **token stake alone** now that the finality firewall's live-path wiring has landed for committee selection (Section 13.5 Honest status) — CPU / Proof-of-Vault work no longer weighs into it, because it is Sybil-weak until PoRep-hardened; making finality influence track demonstrated CPU contribution Sybil-resistantly (once re-admitted) is the PoRep-gated mainnet goal. (Leader selection has no live weighted mechanism yet, pending mainnet leader-scheduling.)
...
**Status.** This section specifies the committee-selection algorithm, and it is now live: on the current single-coordinator testnet, verifier selection runs on the token-only finality weight (`W_fin`) inside the coordinator's pipeline — see §13.5 Honest status.
...
- *Mitigation (live for committee selection — finality firewall, §13.5):* Committee selection is weighted by **AGNTC token stake only** (Section 13.5), so the Sybil cost of dominating committee selection is the **cost of the token stake** — a single, clean axis. Critically, the CPU / Proof-of-Vault leg is *excluded* from committee-selection finality precisely because it is Sybil-weak until PoRep-hardened: cheaply forging storage proofs no longer discounts committee capture. Leader selection has no live weighted mechanism yet (the coordinator is a single fixed proposer), so it carries no live Sybil-selection surface to mitigate; it inherits the firewall once mainnet leader-scheduling ships (§13.5 Honest status). Token stake at risk is additionally subject to slashing (Section 15) and dispute re-verification (Section 8.7).
...
**The same ships-now-vs-future discipline governs consensus finality (the finality firewall).** Just as we forbid present-tense ZK claims above the shipping rung, we forbid claiming that *demonstrated CPU work earns finality influence* before that work is Sybil-resistant. The finality weight for committee selection — `W_fin`, Section 13.5 — is **AGNTC token stake only**, implemented, test-guarded, and now wired into the live coordinator path (Section 13.5 Honest status). Leader selection has no live weighted mechanism at all (the coordinator is a single fixed proposer); it is not a present-tense claim of any kind, weighted or not, pending mainnet leader-scheduling. This split exists because the CPU / Proof-of-Vault leg is Sybil-weak (a possession proof is forgeable until SNARK-/PoRep-hardened — rung (a) and Section 5A.5). CPU-weighted committee selection (the original §13 dual-staking-in-finality vision) remains only a **PoRep-gated mainnet goal**, not a claim that CPU will ever again buy committee seats before that hardening — the consensus analogue of a dated future rung. CPU still earns proportionally more *economically* today (effective stake, Section 14); it no longer buys committee seats now that committee selection runs on `W_fin` live.
...
ZK Agentic Chain targets a block time of 60 seconds (BLOCK_TIME_MS = 60,000). Each block contains up to 50 transactions (MAX_TXS_PER_BLOCK = 50). The design specifies the block proposer (leader) to be selected through the same VRF mechanism used for committee selection, weighted by the **same token-only finality weight** `W_fin` (the finality firewall, Section 13.5) — both leader and committee selection are specified to exclude the Sybil-weak CPU leg until it is PoRep-hardened. **Live status:** committee selection runs this way today (Section 5.5); leader selection does not yet exist as a weighted mechanism of any kind on the testnet — the coordinator is a single, fixed block proposer, and real leader scheduling (VRF- or otherwise-selected) remains explicit mainnet-stage future work (Section 13.5 Honest status).
...
**Property 3: Committee Unbiasability (COM-UNBIAS) — live**
No PPT adversary controlling less than 1/3 of total **finality weight** (staked AGNTC; the token-only `W_fin` of the finality firewall, Section 13.5 Honest status — committee composition is weighted by token stake, not effective stake) can predictably influence committee composition beyond their proportional representation, except with negligible probability. On the current coordinator testnet, committee selection now runs on `W_fin`, so this property's guarantee holds today for committee composition. (It does not yet extend to leader selection, which has no live weighted mechanism of any kind — a single fixed proposer, pending mainnet leader-scheduling.)
...
**Finality (live — token dimension only, committee selection; §13.5).** Committee selection is weighted by **AGNTC token stake alone**, live in the coordinator path as of 2026-07-15 (Honest status, §13.5); leader selection has no live weighted mechanism yet, pending mainnet leader-scheduling. The token weight for *committee-selection finality* is therefore effectively 1.0, not α = 0.40: an attacker needs to acquire a significant fraction of the total *staked AGNTC* to gain meaningful committee representation, and that is the **whole** of the committee-selection Sybil cost. The CPU leg is **deliberately excluded** from committee-selection finality because cheap, Sybil-weak Proof-of-Vault work would otherwise let an adversary buy committee seats by forging storage proofs (one disk presenting as many — Section 5A.5). This is the security purpose of the firewall: we do **not** rely on the CPU axis for committee-selection Sybil-resistance.
...
| Singularity coordinator (testnet trust assumption) | VER-INT, VER-PRIV, COM-UNBIAS (ledger safety intact) — **the finality firewall now makes this clean for committee selection (§13.5 Honest status); the coordinator's trust scope no longer includes committee-selection weighting, though it still produces every block as the sole, unweighted proposer** | State-layer measurement reliability and *earnings* fairness — CPU/storage-proof metering can be biased (inflating an attacker's reward share) until the mainnet committee/on-chain verifier replaces the coordinator (Sections 13.5, 24.3); committee-selection finality itself is no longer exposed to this bias |

#### Sybil Cost Derivation

**Scope (read first — finality firewall).** The following derivation analyzes the cost of dominating selection *when CPU stake weights it* — the pre-firewall (v1.4) model. Under the finality firewall (Section 13.5), CPU stake does **not** weight committee-selection finality — this is now live, so the committee-selection Sybil cost is simply the token cost (the pure-PoS line below, `X`). The dual-axis 2.5× result is therefore the **PoRep-gated mainnet target** — what the cost becomes once committed CPU+disk is Sybil-resistant and re-admitted to the finality weight — and, today, a characterization of *earnings* concentration (reward share still tracks effective stake), plus of leader selection, which has no live weighting to derive a Sybil cost for yet. It is retained, not deleted, because it specifies the design we are hardening toward.
...
**Staking.** Validators must stake AGNTC alongside CPU compute resources to participate in block verification. The staked AGNTC is the **finality weight** that determines **committee selection probability**, live, under the finality firewall (Section 13.5); leader selection has no live weighted mechanism yet (a single fixed proposer on testnet), pending mainnet leader-scheduling. The staked amount also contributes the token component (alpha = 0.40) to the dual-staking effective stake `S_eff`, which determines **reward share**; the CPU leg (beta = 0.60) adds to reward share but not to committee selection.
...
> 2. **Finality weight = `W_fin` = AGNTC token stake only** (online-gated), the *finality firewall*. It governs **committee (verifier) selection** (Section 5.5) **live**, as of 2026-07-15 (§13.5 Honest status); CPU no longer weighs into committee-selection finality. **Leader selection** (Section 7.1) has no live weighted mechanism of any kind yet — the testnet coordinator is a single fixed proposer — so it is pending: neither `S_eff`-weighted today nor yet `W_fin`-weighted.
>
> **Why finality is token-only (security item P1-1).** Finality must be weighted by a Sybil-*resistant* quantity. AGNTC stake is Sybil-resistant (you must actually acquire and lock it); the CPU / Proof-of-Vault leg is **not yet** — a cheap *possession* proof is forgeable until PoRep-hardened (one disk presenting as many — Section 5A.5). If CPU weighted finality, **cheaply corrupting Proof-of-Vault would be a cheap path to consensus capture** — which is why the protocol firewalls finality to token stake, and why, as of 2026-07-15, the live coordinator path weights committee selection by `W_fin`, not `S_eff` (§13.5 Honest status). CPU stays for earnings and for liveness/admission — never for committee-selection finality. **CPU-weighted committee selection — the original §13 dual-staking-in-finality vision — remains only a mainnet GOAL, gated on PoRep-hardening the CPU stake** (Section 13.5). Leader selection (Section 7.1) is a separate, still-open item: it has no live weighted mechanism of either kind — the testnet coordinator proposes every block itself — pending mainnet leader-scheduling. This is stated in the same honesty style as the ZK ladder (§5B.2): committee selection is now a shipped, present-tense claim; leader selection and CPU re-admission remain dated future targets, not present claims.
>
...
**Architectural keystone — the finality firewall (implemented in the consensus module; live for committee selection as of 2026-07-15).** v1.5 specified the finality weight as AGNTC token stake only (`W_fin`), implemented and test-guarded in the consensus module (`test_consensus_firewall.py`). **Honest status (updated 2026-07-15):** the live coordinator's verification-committee selection — the call site inside its block-verification pipeline that a code-verified audit found still on effective-stake weighting — now weights by `W_fin`, the same token-only formula `vrf.select_verifiers` and the whitepaper-concordance `ConsensusSimulator` already used elsewhere in the same codebase. This closes the gap this section previously disclosed, for committee selection. **Leader selection remains unimplemented as a weighted mechanism of any kind:** the testnet coordinator is a single, fixed block proposer — there is no leader rotation to weight, by `S_eff` or `W_fin` — so real leader scheduling, and therefore its firewalling, is explicit mainnet-stage future work (Section 7.1), independent of this fix.
...
Committee (verifier) selection (Section 5.5) is now weighted by `W_fin` in the live coordinator path. Leader selection (Section 7.1) has no live selection mechanism to weight yet — the coordinator proposes every block itself — so it is neither `S_eff`-weighted nor `W_fin`-weighted today; the firewall applies to it automatically once real leader scheduling ships. The CPU / Proof-of-Vault leg of effective stake (Section 13.1, β = 0.60) is now excluded from live committee-selection finality; it continues to feed **earnings** (reward share, Section 14) and liveness/admission, exactly as designed.

*Why (security item P1-1).* The v1.4 edition of this document disclosed the inverse as a known pre-mainnet risk: committee selection then weighted `α·token + β·cpu`, so the CPU leg fed finality, and the clean ledger-safety firewall held *only to the extent the CPU-stake measurement was Sybil-resistant* — which testnet Proof-of-Vault is **not**. A cheap *possession* proof is sybil-, outsourcing-, and generation-attackable (one disk can present as several; Section 5A.5), so a metering-inflation or PoV-corruption attack was a path to bias *committee selection* — a ledger-relevant influence. v1.5 specified the closure; **it is now live for committee selection** — a compromised or biased storage layer can no longer buy committee seats, only degrade state-measurement and earnings fairness. Leader selection carries no analogous residual risk today because it is not weighted by anything at all yet (a single fixed proposer); it will inherit the firewall automatically once leader scheduling is built.

*Mainnet goal (CPU re-admission to finality).* The original §13 vision — CPU contribution *also* weighting committee selection, so that demonstrated work earns consensus influence and a Sybil attack must pay along both axes (the ≈2.5× cost of Section 8) — is **deferred, not abandoned**. It is gated on making the CPU leg Sybil-resistant: items 2–3 of the mitigation roadmap above (a trustless/on-chain verifier replacing the coordinator, plus Proof-of-Replication sealing + timed/keyed challenges). Once committed CPU+disk is measured Sybil-resistantly, the protocol can re-admit a hardened CPU term to the finality weight under governance (Section 21.2). Until then, the honest, un-rounded statement is: **finality for committee selection is token-weighted and live; leader selection has no live weight of any kind, pending mainnet leader-scheduling; CPU-weighted finality (for either) is a PoRep-gated mainnet target for after that.** The residual testnet trust in the Singularity coordinator (next item) therefore bounds *state-measurement and earnings* fairness, and — until real leader scheduling ships — the coordinator's fixed-proposer role itself (cross-referenced at Section 8.8, Section 24.3).
...
| BETA ‡ | 0.60 | CPU weight in effective stake formula (economic / reward-share only). **Note (v1.5 finality firewall):** `S_eff` governs *earnings*; consensus *finality* selection for the committee is token-stake-only (`W_fin = T/T_total`), live in the coordinator path (§13.5 Honest status) — β no longer weights committee selection there. Leader selection has no live weighted mechanism yet, pending mainnet leader-scheduling. ALPHA/BETA values and the `S_eff` formula are unchanged |
...
**Related keystone (finality firewall — coupling removed for committee selection; live).** v1.4 disclosed a CPU-stake → finality coupling here: because effective stake then weighted `α·token + β·cpu`, the CPU leg measured by this same coordinator also fed *committee selection*, giving the storage-measurement trust assumption a bounded influence on finality. **The coupling removal is implemented in the consensus module** (Section 13.5, "Architectural keystone") **and live for committee selection** as of 2026-07-15: committee selection is weighted by **token stake only** (`W_fin`). **Honest status:** the coordinator's block-production pipeline no longer weights committee selection by the effective stake `S_eff`; a compromised or biased storage layer can no longer buy committee seats this way. Leader selection carries no analogous fix yet because it has no live weighted mechanism of any kind (a single fixed proposer) — it inherits the firewall once mainnet leader-scheduling ships. The coordinator's trust scope is accordingly narrower today: state measurement and earnings, plus its role as sole block proposer, but no longer committee-selection weighting. Re-admitting a PoRep-hardened CPU term to the finality weight (so demonstrated work again earns consensus influence, at the dual-axis Sybil cost of Section 8) remains the mainnet goal, gated on the trustless verifier and unique-replica sealing.
...
| **W_fin (finality weight)** | The **finality firewall** quantity: `W_fin(i) = T_i / T_total` (AGNTC **token stake only**, online-gated) — specified v1.5, implemented in the consensus module, and live in the coordinator's committee selection as of 2026-07-15 (Section 5.5). Leader selection (Section 7.1) has no live weighted mechanism yet (single fixed proposer). CPU / Proof-of-Vault work is excluded from `W_fin` — it is Sybil-weak until PoRep-hardened, so weighting finality by it would let a corrupted storage layer buy consensus influence (security item P1-1). Distinct from `S_eff` (earnings). Section 13.5 |
| **Finality firewall** | The rule that consensus *finality selection* — committee selection, and (once it exists) leader selection — is weighted by token stake only (`W_fin`), never by the Sybil-weak CPU/PoV leg: specified v1.5, implemented in the consensus module, and live for committee selection under the coordinator as of 2026-07-15. Leader selection has no live weighted mechanism yet (single fixed proposer), pending mainnet leader-scheduling. Re-admitting a PoRep-hardened CPU term to finality is the mainnet goal. Sections 13.5, 8.3 |
