...
describe("gameStore — lattice/blocknode state", () => {
  ...
  it("claimBlocknode rejects cross-faction claim when currentUserFaction is set", () => {
    ...
    expect(result).toBe(false);
    expect(useGameStore.getState().blocknodes["block-0-treasury"].ownerId).toBeNull();
  });
  ...
  it("claimBlocknode returns false when currentUserFaction is set (arm nodes are faction infrastructure, not user territory)", () => {
    ...
    const result = useGameStore.getState().claimBlocknode("block-0-community", "user-001");
    expect(result).toBe(false);
    expect(useGameStore.getState().blocknodes["block-0-community"].ownerId).toBeNull();
  });
  ...
  it("claimBlocknode does not allow ring-1 claims (arm nodes are faction infrastructure, not user territory)", () => {
    ...
    const ring0Result = useGameStore.getState().claimBlocknode("block-0-community", "user-001");
    expect(ring0Result).toBe(false);
    const ring1Result = useGameStore.getState().claimBlocknode("block-1-community", "user-001");
  ...
  it("secureBlocknode does NOT spawn new nodes", () => {
    ...
    useGameStore.getState().secureBlocknode("block-0-community", 9999);
...
//   community=(0,-1)  treasury=(1,0)  founder=(0,1)  pro-max=(-1,0)
// Cell (0,-2) is directly above community arm — unambiguously community Voronoi territory.
...
describe("gameStore — grid node territory (mineGridNode / claimGridNode)", () => {
  ...
  it("mineGridNode mines a cell adjacent to faction arm", () => {
    // (0,-2) is adjacent to community arm node (0,-1) and clearly in community territory
  ...
  it("mineGridNode returns false for arm cell position", () => {
    // (0,-1) is the community arm node — cannot be mined as territory
    const result = useGameStore.getState().mineGridNode(0, -1);
    expect(result).toBe(false);
  });
  ...
  it("mineGridNode returns false when cell is in another faction's territory", () => {
    // (1,-1) is equidistant from community(0,-1) and treasury(1,0) at d²=2 each,
    // but we test a clearly treasury cell: (2,0) → d² to treasury(1,0)=1 vs community(0,-1)=5
