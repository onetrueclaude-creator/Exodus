...
from agentic.params import MERKLE_TREE_DEPTH, TOTAL_SUPPLY, DIST_COMMUNITY
from agentic.testnet.genesis import create_genesis
from agentic.galaxy.mining import COMMUNITY_POOL_TOTAL
...
def chart_pool_depletion():
    """Chart: Community pool depletion over blocks."""
    ...
    for i in range(500):
        blocks.append(i)
        remaining.append(g.community_pool.remaining)
        total_mined.append(g.mining_engine.total_rewards_distributed)
        g.mining_engine.compute_block_yields(claims_input)

    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 5))

    ax1.plot(blocks, [r / 1e6 for r in remaining], color="#00D4FF", linewidth=2)
    ax1.set_xlabel("Blocks Mined", fontsize=12)
    ax1.set_ylabel("Pool Remaining (M AGNTC)", fontsize=12)
    ax1.set_title("Community Pool Depletion", fontsize=14, fontweight="bold")
    ax1.grid(True, alpha=0.3)

    ax2.plot(blocks, [t for t in total_mined], color="#8B5CF6", linewidth=2)
    ax2.set_xlabel("Blocks Mined", fontsize=12)
    ax2.set_ylabel("Total Mined (AGNTC)", fontsize=12)
    ax2.set_title("Cumulative Mining Rewards", fontsize=14, fontweight="bold")
    ax2.grid(True, alpha=0.3)
