...
class TransferTx:
    ...
    def build(
        ...
        input_nullifiers = [
            rec.nullifier(sender_keys["spending_key"])
        ...
        # Recipient record
        recipient_tag_nonce = int.from_bytes(os.urandom(4), "big")
        ...
        if change > 0:
            sender_tag_nonce = int.from_bytes(os.urandom(4), "big")
            sender_tag = hash_tag(
...
def validate_mint(tx: MintTx, state: LedgerState) -> TxResult:
    ...
    """
    tag = hash_tag(tx.recipient_viewing_key, tx.program_id, state.record_count)
    record = Record(
        owner=tx.recipient,
        data=[tx.amount],
        nonce=os.urandom(32),
...
def validate_stake(tx: StakeTx, state: LedgerState, stake_registry: StakeRegistry) -> TxResult:
    ...
    if change > 0:
        change_tag_nonce = int.from_bytes(os.urandom(4), "big")
...
def validate_unstake(tx: UnstakeTx, state: LedgerState, stake_registry: StakeRegistry) -> TxResult:
    ...
    # 3. Create output record for unstaked amount
    tag_nonce = int.from_bytes(os.urandom(4), "big")
...
def validate_birth(
    ...
    star_tag = hash_tag(tx.staker_viewing_key, BIRTH_PROGRAM_ID, state.record_count)
    star_record = Record(
        owner=tx.staker,
