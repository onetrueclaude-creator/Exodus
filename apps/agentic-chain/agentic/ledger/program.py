...
class MintProgram(Program):
    ...
    def execute(self, inp: ProgramInput, state: LedgerState) -> ProgramOutput:
        ...
        tag = hash_tag(viewing_key, self.program_id, state.record_count)
        record = Record(
            owner=recipient,
            data=[amount],
            nonce=os.urandom(32),
...
class TransferProgram(Program):
    ...
    def execute(self, inp: ProgramInput, state: LedgerState) -> ProgramOutput:
        ...
        # Recipient record
        tag = hash_tag(recipient_viewing_key, self.program_id, tag_counter)
        ...
        if change > 0:
            ...
            tag = hash_tag(sender_viewing_key, self.program_id, tag_counter + 1)
            outputs.append(Record(
                owner=inp.caller,
                data=[change],
                nonce=os.urandom(32),
...
class StakeProgram(Program):
    ...
    def execute(self, inp: ProgramInput, state: LedgerState) -> ProgramOutput:
        ...
        tag = hash_tag(inp.caller, self.program_id, state.record_count)
        ...
        if change > 0:
            sender_viewing_key = inp.arguments.get("sender_viewing_key", b"")
            change_tag = hash_tag(sender_viewing_key, TRANSFER_PROGRAM_ID, state.record_count + 1)
            outputs.append(Record(
                owner=inp.caller,
                data=[change],
                nonce=os.urandom(32),
